...
def run_integration():
    ...
    try:
        ...
        print(f"  WARN RAG timeout: {type(e).__name__}")
        print("  SKIP RAG tests")
    ...
    print("=" * 60)
    try:
        code, body = get(f"{FRONTEND}/api/search/semantic?q=artificial+intelligence&top_k=3", timeout=30)
        sem = json.loads(body)
        ...
        print(f"  WARN semantic search timeout: {type(e).__name__}")
        print("  SKIP semantic search")

    print()
    print("=" * 60)
    print("  СЦЕНАРИЙ 11: Фильтры по источнику и дате")
    print("=" * 60)
    code, body = get(f"{FRONTEND}/api/articles?source=techcrunch.com&page_size=3")
    d = json.loads(body)
    check("source_filter_works", code == 200 and d["total"] > 0, f"total={d['total']}")
    check("source_filter_correct", all(i["source"] == "techcrunch.com" for i in d["items"]))

    code, body = get(f"{FRONTEND}/api/articles?date_from=2026-03-21&page_size=3")
    d = json.loads(body)
    check("date_from_filter_works", code == 200 and d["total"] > 0, f"total={d['total']}")

    code, body = get(f"{FRONTEND}/api/articles?date_to=2026-03-20&page_size=3")
    d = json.loads(body)
    check("date_to_filter_works", code == 200 and d["total"] > 0, f"total={d['total']}")

    code, body = get(f"{FRONTEND}/api/meta")
    meta = json.loads(body)
    check("meta_has_sources", len(meta.get("sources", [])) > 0, f"sources={len(meta['sources'])}")
    check("meta_has_languages", len(meta.get("languages", [])) > 0, f"languages={meta['languages']}")
